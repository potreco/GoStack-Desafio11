...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      ...
      setExtras(
        apiFood.extras.map((extra: Omit<Extra, 'quantity'>) => ({
          ...extra,
          quantity: 0,
        })),
      );
  ...
  const cartTotal = useMemo(() => {
    const extraTotalValue = extras.reduce((accumulator, extra) => {
      return accumulator + extra.quantity * extra.value;
    }, 0);

    const foodTotalValue = food.price;

    return formatValue((extraTotalValue + foodTotalValue) * foodQuantity);
